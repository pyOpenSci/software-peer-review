...
If you are building a pure Python package, your setup may be simple.
...
To support the myriad uses of Python, there are many ways
to create a Python package.

```{admonition} Python supports extensions written in other languages
:class: info

The spatial data tool stack is a common example of tools that
...
that will make the packaging landscape far easier to navigate.
...
to packaging through tutorials, documentation and guides as well as its peer review process. We do this by:
...
We also endeavor to help maintainers use a similar infrastructure for
...
Through our peer review, we also help maintainer with outreach
...
It is easy to find multiple packages on `PyPI` that perform that same tasks
(or overlapping tasks). When you submit a package to us, we ask that you cross reference
other packages in our system that may perform similar purposes. This check will
...
In some cases just adding information to your **README.md** about how your
package varies from others in the ecosystem is extremely valuable to users.

* [See our discussion of package overlap for more](package-overlap)

### 4. Support long term maintenance of vetted tools

Most maintainers are not renumerated for their efforts and thus work on tools
in their free time. So what happens when a maintainer of a commonly used tool wants
to step down? We help maintainers identify someone new to take over the reigns.
...
enhance their package infrastructure. Examples of this include moving
...
may be, it is always easier to work when you have community support.
...
of both differing technical background and skill as well as varying cultural background.
